        ...
        <div className="space-y-1 max-h-48 overflow-y-auto">
          {clusters.length === 0 && !showCreate && (
            <p className="text-xs text-muted-foreground px-2 py-1.5">
              {t({ fr: 'Aucun cluster', en: 'No clusters', es: 'Sin clusters' }).primary}
            </p>
          )}
          {clusters.map(c => (
            <button
            ...
            {showCreate ? (
              <div className="flex gap-1.5 p-1">
                <Input
                  ...
                  placeholder={t({ fr: 'Nom…', en: 'Name…', es: 'Nombre…' }).primary}
              ...
              >
                ...
                {t({ fr: 'Nouveau cluster', en: 'New cluster', es: 'Nuevo cluster' }).primary}
              </button>
            )}
          </>
        )}
