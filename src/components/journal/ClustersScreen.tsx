    ...
    <div className="min-h-screen flex flex-col px-6 py-8">
      ...
      <div className="text-center mb-6">
        <h2 className="font-serif text-3xl text-foreground flex items-center justify-center gap-2">
          ...
          {bilingual({ fr: 'Mes Clusters', en: 'My Clusters', es: 'Mis Grupos', ja: 'マイ クラスター', 'zh-Hans': '我的分组', 'zh-Hant': '我的分組' })}
        </h2>
        <p className="text-muted-foreground text-sm mt-1">
          {t(
            { fr: 'Regroupez vos pensées librement.', en: 'Group your thoughts freely.', es: 'Agrupa tus pensamientos libremente.', ja: '思考を自由にまとめましょう。', 'zh-Hans': '把想法自由地归类。', 'zh-Hant': '把想法自由地歸類。' }
          ).primary}
        </p>
      </div>
      ...
      {showCreate && (
        <div className="max-w-lg mx-auto w-full mb-6 animate-fade-in-up">
          <div className="flex gap-2">
            <Input
              placeholder={t({ fr: 'Nom du cluster…', en: 'Cluster name…', es: 'Nombre del grupo…', ja: 'クラスター名…', 'zh-Hans': '分组名称…', 'zh-Hant': '分組名稱…' }).primary}
              value={newTitle}
            ...
            <Button onClick={handleCreate} disabled={!newTitle.trim() || creating} size="sm">
              {creating ? '…' : t({ fr: 'Créer', en: 'Create', es: 'Crear', ja: '作成', 'zh-Hans': '创建', 'zh-Hant': '建立' }).primary}
            </Button>
          </div>
        </div>
      )}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-muted-foreground">
              {t(
                { fr: 'Pas encore de clusters. Créez-en un pour regrouper vos pensées.', en: 'No clusters yet. Create one to group your thoughts.', es: 'Aún no hay grupos. Crea uno para agrupar tus pensamientos.', ja: 'まだクラスターがありません。作成して思考をまとめましょう。', 'zh-Hans': '还没有分组。新建一个，把想法收拢起来。', 'zh-Hant': '還沒有分組。新增一個，把想法收攏起來。' }
              ).primary}
            </p>
          </div>
        ) : (
